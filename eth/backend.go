...
func New(ctx context.Context, stack *node.Node, config *ethconfig.Config, logger log.Logger) (*Ethereum, error) {
	...
	if config.NetworkID == chain.GnosisChainID || config.NetworkID == chain.HoleskyChainID || config.NetworkID == chain.GoerliChainID {
		// Read the jwt secret
		...
		executionEngine, err = executionclient.NewExecutionClientDirect(eth1_chain_reader.NewChainReaderEth1(chainConfig, executionRpc, 1000), nil)
		if err != nil {
			return nil, err
		}
	}

	// If we choose to run the internal consensus layer (Caplin)
	if config.InternalCL && clparams.EmbeddedSupported(config.NetworkID) {
		caplinService, err := NewCaplinService(
			ctx,
			logger,
			config,
			executionEngine,
		...
		backend.caplinService = caplinService
	}
...
func (s *Ethereum) Stop() error {
	...
	// Stop Caplin embedded consensus layer
	if s.caplinService != nil {
