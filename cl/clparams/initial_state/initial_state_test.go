...
func TestMainnet(t *testing.T) {
	state, err := initial_state.GetGenesisState(clparams.NetworkType(chain.MainnetChainID))
...
func TestSepolia(t *testing.T) {
	state, err := initial_state.GetGenesisState(clparams.NetworkType(chain.SepoliaChainID))
	require.NoError(t, err)
	root, err := state.HashSSZ()
	require.NoError(t, err)
