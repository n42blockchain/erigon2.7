		...
		mockValidators := []*cltypes.ValidatorRegistration{
			{
				Message: cltypes.ValidatorRegistrationMessage{
					FeeRecipient: libcommon.HexToAddress("0xAbcF8e0d4e9587369b2301D0790347320302cc09"),
					GasLimit:     "456",
					Timestamp:    "456",
			...
			{
				Message: cltypes.ValidatorRegistrationMessage{
					FeeRecipient: libcommon.HexToAddress("0x123"),
					GasLimit:     "123",
					Timestamp:    "123",
...
func TestGetHeader(t *testing.T) {
	...
	expectMethod := http.MethodGet
	mockSlot := int64(123)
	mockParentHash := libcommon.HexToHash("0x1234567")
...
func newBytes48FromString(s string) libcommon.Bytes48 {
	bytes := libcommon.Hex2Bytes(s)
	var b libcommon.Bytes48
	copy(b[:], bytes)
	return b
}

func newBytes96FromString(s string) libcommon.Bytes96 {
	bytes := libcommon.Hex2Bytes(s)
	var b libcommon.Bytes96
	copy(b[:], bytes)
	return b
}
