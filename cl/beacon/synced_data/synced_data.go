...
func (s *SyncedDataManager) ViewHeadState(fn ViewHeadStateFn) error {
	_, synced := s.headRoot.Load().(libcommon.Hash)
	if !s.enabled || !synced {
		return ErrNotSynced
	}
	s.mu.RLock()
...
func (s *SyncedDataManager) HeadRoot() libcommon.Hash {
	if !s.enabled {
		return libcommon.Hash{}
	}
	root, ok := s.headRoot.Load().(libcommon.Hash)
	if !ok {
		return libcommon.Hash{}
	}
	return root
...
func (s *SyncedDataManager) ValidatorPublicKeyByIndex(index int) (libcommon.Bytes48, error) {
	s.accessLock.RLock()
	defer s.accessLock.RUnlock()
	if s.headState == nil {
		return libcommon.Bytes48{}, ErrNotSynced
	}
	return s.headState.ValidatorPublicKey(index)
}

func (s *SyncedDataManager) ValidatorIndexByPublicKey(pubkey libcommon.Bytes48) (uint64, bool, error) {
	s.accessLock.RLock()
	defer s.accessLock.RUnlock()
	if s.headState == nil {
...
func (s *SyncedDataManager) HistoricalRootElementAtIndex(index int) (libcommon.Hash, error) {
	s.accessLock.RLock()
	defer s.accessLock.RUnlock()
	if s.headState == nil {
		return libcommon.Hash{}, ErrNotSynced
	}
	if s.headState.HistoricalRootsLength() <= uint64(index) {
		return libcommon.Hash{}, errors.New("HistoricalRootElementAtIndex: index out of range")
	}
