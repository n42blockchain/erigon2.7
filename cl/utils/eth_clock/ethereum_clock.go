...
type EthereumClock interface {
	...
	CurrentForkDigest() (libcommon.Bytes4, error) // ComputeForkDigest
	NextForkDigest() (libcommon.Bytes4, error)    // ComputeForkDigest
	NextForkEpochIncludeBPO() uint64
	ForkId() ([]byte, error)                                               // ComputeForkId
	LastFork() (libcommon.Bytes4, error)                                      // GetLastFork
	StateVersionByForkDigest(libcommon.Bytes4) (clparams.StateVersion, error) // ForkDigestVersion
	StateVersionByEpoch(uint64) clparams.StateVersion
	//ComputeForkDigestForVersion(currentVersion libcommon.Bytes4) (digest libcommon.Bytes4, err error)
	ComputeForkDigest(epoch uint64) (digest libcommon.Bytes4, err error) // new in fulu
...
func forkList(schedule map[libcommon.Bytes4]clparams.VersionScheduleEntry) (f []forkNode) {
...
type ethereumClockImpl struct {
	...
	forkDigestToVersion   map[libcommon.Bytes4]clparams.StateVersion
}

func NewEthereumClock(genesisTime uint64, genesisValidatorsRoot libcommon.Hash, beaconCfg *clparams.BeaconChainConfig) EthereumClock {
	impl := &ethereumClockImpl{
		genesisTime:           genesisTime,
		beaconCfg:             beaconCfg,
		genesisValidatorsRoot: genesisValidatorsRoot,
		forkDigestToVersion:   make(map[libcommon.Bytes4]clparams.StateVersion),
	}
...
func (t *ethereumClockImpl) CurrentForkDigest() (libcommon.Bytes4, error) {
	currentEpoch := t.GetCurrentEpoch()
	return t.ComputeForkDigest(currentEpoch)
}

func (t *ethereumClockImpl) NextForkDigest() (libcommon.Bytes4, error) {
...
func (t *ethereumClockImpl) LastFork() (libcommon.Bytes4, error) {
	currentEpoch := t.GetCurrentEpoch()
...
func (t *ethereumClockImpl) StateVersionByForkDigest(digest libcommon.Bytes4) (clparams.StateVersion, error) {
...
func (t *ethereumClockImpl) computeForkDigestForVersion(currentVersion libcommon.Bytes4) (digest libcommon.Bytes4, err error) {
...
func (t *ethereumClockImpl) ComputeForkDigest(epoch uint64) (digest libcommon.Bytes4, err error) {
...
func (t *ethereumClockImpl) GenesisValidatorsRoot() libcommon.Hash {
...
func computeForkDataRoot(version [4]byte, genesisValidatorsRoot libcommon.Hash) libcommon.Hash {
