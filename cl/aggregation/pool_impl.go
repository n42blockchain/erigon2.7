...
type aggregationPoolImpl struct {
	...
	aggregates     map[libcommon.Hash]*solid.Attestation // don't need this anymore after electra upgrade
...
func NewAggregationPool(
	...
	p := &aggregationPoolImpl{
		...
		netConfig:             netConfig,
		aggregatesLock:        sync.RWMutex{},
		aggregates:            make(map[libcommon.Hash]*solid.Attestation),
...
func (p *aggregationPoolImpl) GetAggregatationByRoot(root libcommon.Hash) *solid.Attestation {
	p.aggregatesLock.RLock()
	defer p.aggregatesLock.RUnlock()
	return p.aggregates[root]
}

func (p *aggregationPoolImpl) GetAggregatationByRootAndCommittee(root libcommon.Hash, committeeIndex uint64) *solid.Attestation {
	p.aggregatesLock.RLock()
	defer p.aggregatesLock.RUnlock()
	key := keyAggrInCommittee{
		dataRoot:       root,
