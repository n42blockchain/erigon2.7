...
type SyncContributionPool interface {
	...
	GetSyncContribution(slot, subcommitteeIndex uint64, beaconBlockRoot libcommon.Hash) *cltypes.Contribution
	// Obtain the sync aggregate for the sync messages pointing to a given beacon block root.
	GetSyncAggregate(slot uint64, beaconBlockRoot libcommon.Hash) (*cltypes.SyncAggregate, error)
}
