...
var Eip4844MerkleProof = spectest.HandlerFunc(func(t *testing.T, root fs.FS, c spectest.TestCase) (err error) {
	...
	require.NoError(t, err)
	proofHashes := make([]libcommon.Hash, len(branch))
	for i := range branch {
		proofHashes[i] = libcommon.Hash(branch[i])
	}
	...
	for i := 0; i < beaconBody.BlobKzgCommitments.Len(); i++ {
		...
		for j := 0; j < len(proof); j++ {
			commitmentInclusionProof.Set(j, libcommon.Hash(proof[j]))
		}
		require.True(t, cltypes.VerifyCommitmentInclusionProof(libcommon.Bytes48(*beaconBody.BlobKzgCommitments.Get(i)), commitmentInclusionProof, uint64(i), c.Version(), bodyRoot))
	}
	return nil
})
