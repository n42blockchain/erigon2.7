...
func TestSentinelGossipOnHardFork(t *testing.T) {
	...
	ethClock.EXPECT().CurrentForkDigest().DoAndReturn(func() (libcommon.Bytes4, error) {
		if hardFork.Load() {
			forkDigest := libcommon.Bytes4{0x00, 0x00, 0x00, 0x01}
			return forkDigest, nil
		}
		return libcommon.Bytes4{0x00, 0x00, 0x00, 0x00}, nil
	}).AnyTimes()
	...
	ethClock.EXPECT().NextForkDigest().DoAndReturn(func() (libcommon.Bytes4, error) {
		if hardFork.Load() {
			return libcommon.Bytes4{0x00, 0x00, 0x00, 0x02}, nil
		}
		return libcommon.Bytes4{0x00, 0x00, 0x00, 0x01}, nil
	}).AnyTimes()
