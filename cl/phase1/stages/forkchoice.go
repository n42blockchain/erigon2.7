...
func computeAndNotifyServicesOfNewForkChoice(ctx context.Context, logger log.Logger, cfg *Cfg) (headSlot uint64, headRoot libcommon.Hash, err error) {
	if err = cfg.syncedData.ViewHeadState(func(prevHeadState *state.CachingBeaconState) error {
		...
		if errors.Is(err, synced_data.ErrNotSynced) {
			...
			if err != nil {
				return 0, libcommon.Hash{}, fmt.Errorf("failed to get head: %w", err)
			}
		} else {
			return 0, libcommon.Hash{}, fmt.Errorf("failed to get head: %w", err)
		}
...
func updateCanonicalChainInTheDatabase(ctx context.Context, tx kv.RwTx, headSlot uint64, headRoot libcommon.Hash, cfg *Cfg) error {
...
func emitHeadEvent(cfg *Cfg, headSlot uint64, headRoot libcommon.Hash, headState *state.CachingBeaconState) error {
...
func emitNextPaylodAttributesEvent(cfg *Cfg, headSlot uint64, headRoot libcommon.Hash, s *state.CachingBeaconState) error {
	...
	payloadAttributes := engine_types.PayloadAttributes{
		...
		SuggestedFeeRecipient: (libcommon.Address{}), // We can not know this ahead of time
...
func postForkchoiceOperations(ctx context.Context, tx kv.RwTx, logger log.Logger, cfg *Cfg, headSlot uint64, headRoot libcommon.Hash) error {
...
func preCacheNextShuffledValidatorSet(ctx context.Context, logger log.Logger, cfg *Cfg, b *state.CachingBeaconState) {
	...
	go func() {
		...
		log.Info("Pre-cached shuffled set", "epoch", nextEpoch, "len", len(shuffledIndicies), "mix", libcommon.Hash(mix))
	}()
}
