...
type InMemoryPublicKeysRegistry struct {
	basePublicKeyRegistry []libcommon.Bytes48
	statesRegistry        map[solid.Checkpoint][]libcommon.Bytes48
...
func NewInMemoryPublicKeysRegistry() *InMemoryPublicKeysRegistry {
	return &InMemoryPublicKeysRegistry{
		basePublicKeyRegistry: make([]libcommon.Bytes48, 0),
		statesRegistry:        make(map[solid.Checkpoint][]libcommon.Bytes48),
	}
}
...
// 	VerifyAggregateSignature(blockRoot libcommon.Hash, slot uint64, pubkeysIdxs []uint64, message []byte, signature libcommon.Bytes96) error
//  AddState(epoch uint64, blockRoot libcommon.Hash, s abstract.BeaconState)
// 	Prune(epoch uint64)
// }
...
func (r *InMemoryPublicKeysRegistry) ResetAnchor(s abstract.BeaconState) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.statesRegistry = make(map[solid.Checkpoint][]libcommon.Bytes48)        // reset the registry
	r.basePublicKeyRegistry = make([]libcommon.Bytes48, s.ValidatorLength()) // reset the public keys registry
...
func (r *InMemoryPublicKeysRegistry) VerifyAggregateSignature(checkpoint solid.Checkpoint, pubkeysIdxs *solid.RawUint64List, message []byte, signature libcommon.Bytes96) (bool, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *InMemoryPublicKeysRegistry) AddState(checkpoint solid.Checkpoint, s abstract.BeaconState) {
	r.mu.Lock()
	defer r.mu.Unlock()
	statePublicKeys := make([]libcommon.Bytes48, s.ValidatorLength()-len(r.basePublicKeyRegistry))
