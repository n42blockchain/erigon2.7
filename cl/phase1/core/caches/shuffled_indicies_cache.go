...
func (c *ShuffledIndiciesCache) Get(epoch uint64, root libcommon.Hash) ([]uint64, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *ShuffledIndiciesCache) Put(epoch uint64, root libcommon.Hash, shuffledSet []uint64) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if len(c.list) == 0 {
		return
