...
func (b *blockCollector) Flush(ctx context.Context) error {
	...
	if err := b.collector.Load(nil, "", func(k, v []byte, table etl.CurrentTableReader, next etl.LoadNextFunc) error {
		...
		parentRoot := libcommon.BytesToHash(v[1:33])
		requestsHash := libcommon.Hash{}
		if version >= clparams.ElectraVersion {
			// get the requests hash
			requestsHash = libcommon.BytesToHash(v[33:65])
			v = v[65:]
		} else {
			v = v[33:]
		}
...
func encodeBlock(payload *cltypes.Eth1Block, parentRoot libcommon.Hash, executionRequestsList []hexutil.Bytes) ([]byte, error) {
