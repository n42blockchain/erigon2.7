...
func (t *blsToExecutionChangeTestSuite) SetupTest() {
	...
	t.operationsPool = &pool.OperationsPool{
		BLSToExecutionChangesPool: pool.NewOperationPool[libcommon.Bytes96, *cltypes.SignedBLSToExecutionChange](10, "blsToExecutionChangesPool"),
	}
...
func (t *blsToExecutionChangeTestSuite) TestProcessMessage() {
	mockMsg := &SignedBLSToExecutionChangeForGossip{
		SignedBLSToExecutionChange: &cltypes.SignedBLSToExecutionChange{
			Message: &cltypes.BLSToExecutionChange{
				ValidatorIndex: 1,
				From:           libcommon.Bytes48{1, 2, 3, 4, 5, 6},
				To:             libcommon.Address{3, 2, 1},
			},
	...
	tests := []struct {
		...
		// 		mockNewWc := libcommon.Hash{byte(t.beaconCfg.ETH1AddressWithdrawalPrefixByte)}
